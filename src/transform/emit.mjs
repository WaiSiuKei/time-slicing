...
export function Emitter(contextId) {
...
Ep.makeTaskRun = function (expr, async) {
  const t = util.getTypes();
  t.assertExpression(expr);
  if (expr) {
    expr = this.insertedLocs.has(expr)
      ? expr
      : t.cloneDeep(expr);
  }
  ...
  return t.returnStatement(
    t.callExpression(
      this.contextProperty('run'),
      [t.arrowFunctionExpression([], expr, async)]
    )
  );
};
 ...
 *
 * @param path
 * @param {boolean} ignoreResult
 * @param {object} opts
 * @param {boolean} opts.async
 * @param {boolean} opts.mark
 * @returns {any}
 */
Ep.explodeExpression = function (path, ignoreResult, opts = {}) {
  const t = util.getTypes();
  let expr = path.node;
  if (expr) {
    t.assertExpression(expr);
  ...
  // If any child contains a splittable, then any sibling subexpressions will almost
  ...
  switch (expr.type) {
    ...
    case 'CallExpression':
      ...
      let hasSplittableArgs = argsPath.some(
        argPath => meta.containsSplittable(argPath.node)
      );
      ...
      if (t.isMemberExpression(calleePath.node)) {
        ...
        newCallee = self.explodeViaTempVar(null, calleePath, hasSplittableArgs);
      ...
      if (hasSplittableArgs) {
        newArgs = argsPath.map(argPath => self.explodeViaTempVar(null, argPath, hasSplittableArgs));
